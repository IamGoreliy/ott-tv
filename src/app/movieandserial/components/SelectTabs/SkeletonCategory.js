...
export const SkeletonCategory = ({title, windowSize}) => {
    const [whatNumPage, setWhatNumPage] = useState(1);
    const [dataRender, setDataRender] = useState([]);

    useEffect(() => {
        if (windowSize >= 901) {
            const result = auxiliaryFnForPaginationList(listForCategory, whatNumPage);
            setDataRender(result);
        } else {
            setDataRender(listForCategory);
        }

    }, [whatNumPage, windowSize]);
